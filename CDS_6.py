...
def clear_results():
    ...
    lbl_rayleigh_val.config(text="---")
    lbl_N_val.config(text="---")
    lbl_fzp_na_val.config(text="---")
    lbl_theta_val.config(text="---")
...
def calculate():
    ...
    try:
        ...
        resolution_nm = float(entry_resolution.get())
        ...
        if resolution_nm <= 0:
            messagebox.showerror("參數錯誤", "設計解析度 Resolution 必須大於 0")
            clear_results()
            return
        theta_rad = lambda_nm / resolution_nm # Diffraction angle in radians

        # na1 in rad from zp_diameter_um in um and distance in mm
        na1_geometric = zp_diameter_um / (2.0 * distance * 1000.0)
        na1 = na1_geometric - theta_rad
        ...
        if not (0 < na1 < 1): # 驗證計算出的 na1
            messagebox.showerror("參數錯誤", f"計算出的可用數值孔徑 NA1 ({na1:.4f} rad) 無效，必須介於 0 和 1 之間。請檢查輸入參數，特別是解析度與波帶片幾何。")
            clear_results()
            return
        ...
        lbl_rayleigh_val.config(text=f"{rayleigh_nm:.3f}")
        lbl_N_val.config(text=f"{N:.0f}")
        lbl_fzp_na_val.config(text=f"{fzp_na_mrad:.4f}")
        lbl_theta_val.config(text=f"{theta_mrad:.5f}")
...
entry_source_size.insert(0, "1000")
...
entry_resolution = ttk.Entry(frame_input, width=18)
entry_resolution.insert(0, "200")
...
lbl_rayleigh_val = ttk.Label(frame_remarks, text="---", font=("Consolas", 10))
lbl_N_val = ttk.Label(frame_remarks, text="---", font=("Consolas", 10))
lbl_fzp_na_val = ttk.Label(frame_remarks, text="---", font=("Consolas", 10))
lbl_theta_val = ttk.Label(frame_remarks, text="---", font=("Consolas", 10))
...
ttk.Label(frame_input, text="光源大小 (μm):").grid(row=2, column=0, sticky="w", pady=5, padx=5)
entry_source_size.grid(row=2, column=1, pady=5, padx=5)
ttk.Label(frame_input, text="波帶片樣品距離 Distance (mm):").grid(row=3, column=0, sticky="w", pady=5, padx=5)
entry_distance.grid(row=3, column=1, pady=5, padx=5)
ttk.Label(frame_input, text="波帶片直徑 (μm):").grid(row=4, column=0, sticky="w", pady=5, padx=5)
entry_zp_diameter.grid(row=4, column=1, pady=5, padx=5)
ttk.Label(frame_input, text="光源能量 Energy (eV):").grid(row=5, column=0, sticky="w", pady=5, padx=5)
entry_energy.grid(row=5, column=1, pady=5, padx=5)
ttk.Label(frame_input, text="波帶片最外層厚 delta r (nm):").grid(row=6, column=0, sticky="w", pady=5, padx=5)
entry_delta_r.grid(row=6, column=1, pady=5, padx=5)
ttk.Label(frame_input, text="設計解析度 Resolution (nm):").grid(row=7, column=0, sticky="w", pady=5, padx=5)
entry_resolution.grid(row=7, column=1, pady=5, padx=5)
...
ttk.Label(frame_remarks, text="FZP 理論解析度 Rayleigh (nm):").grid(row=5, column=0, sticky="w", pady=5, padx=5)
lbl_rayleigh_val.grid(row=5, column=1, sticky="w", pady=6, padx=5)
...
ttk.Label(frame_remarks, text="FZP NA值 (mrad):").grid(row=7, column=0, sticky="w", pady=5, padx=5)
lbl_fzp_na_val.grid(row=7, column=1, sticky="w", pady=6, padx=5)
ttk.Label(frame_remarks, text="設計解析度的繞射角 theta (mrad):").grid(row=8, column=0, sticky="w", pady=5, padx=5)
lbl_theta_val.grid(row=8, column=1, sticky="w", pady=6, padx=5)
