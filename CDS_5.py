...
def calculate():
    ...
    try:
        ...
        lbl_na1_val.config(text=f"{na1 * 1000:.8f}")
        lbl_na2_val.config(text=f"{na2 * 1000:.8f}")
        lbl_rin_val.config(text=f"{r_in:.8f}")
        lbl_rout_val.config(text=f"{r_out:.8f}")
...
ttk.Label(frame_remarks, text="環寬 Width (%):").grid(row=1, column=0, sticky="w", pady=5, padx=5)
...
ttk.Label(frame_remarks, text="Beam Stop 直徑 (μm):").grid(row=3, column=0, sticky="w", pady=5, padx=5)
lbl_bs_diam_val.grid(row=3, column=1, sticky="w", pady=6, padx=5)
